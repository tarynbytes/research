...
class Run():
    ...
    def organize_parsed_data(self, file_lines):
        summ, start, stop, n, urls, tabs, organized = 0, 0, 0, 1, [] ,[], []
        '''
    ...
    def avg_dload_time(self):
        ...
        for i, line in enumerate(self._organized):
            if int(line._status) == 1:
                ...
                print(f"URL: {line._url}\tTab ID: {line._tabid}\tIndex: {i}\t Started: {line._timestamp}")
            if int(line._status) == 2:
                stop = int(line._timestamp)
                print(f"URL: {line._url}\tTab ID: {line._tabid}\tIndex: {i}\t Stopped: {line._timestamp}")
        #print()
