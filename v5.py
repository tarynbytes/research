...
class Log():
    ...
    def __str__(self):
        return f"timestamp: {self._timestamp}\tuserid: {self._userid}\ttab_id: {self._tabid}\turl: {self._url}\tstatus: {self._status}"
...
class Sessions():
    ...
    def set_unique_sessions(self):
        for key, value in self.unique_userids.items():
            for val in value:
    ...
    @_sorted.setter
    def set_sorted(self):
        for key, value in self._unique_sessions.items():
            self._sorted = sorted(value, key=lambda val: val.status)
...
class Analyzer():
    ...
    def avg_dload_time_each_user(self):
        ...
        for key, value in self._sessions.unique_sessions.items():
            sessions_sorted = sorted(value,key=lambda val: val.status)
            self._pp.pprint(sessions_sorted)
...
def main() -> int:
    ...
    print("\nUNIQUE USERIDS:")
    pp.pprint(sessions.unique_userids.keys())
    pp.pprint(sessions.unique_sessions)
    print()
